...
// TODO: there are so many modifications, so that it is not feasible to have the
// same features and upstream and track that progress, just make a file, that
// derives from it, and state the origin
...
#define ZVEC_DEFINE_VEC_TYPE_EXTENDED(T, Name)                                 \
  ...
  typedef T ZVEC_ELEMENT_TYPENAME(Name);                                       \
                                                                               \
  ZVEC_FUN_ATTRIBUTES [[nodiscard]] ZVEC_TYPENAME(Name)                        \
      zvec_init_capacity_##Name(size_t cap);                                   \
                                                                               \
  ZVEC_FUN_ATTRIBUTES [[nodiscard]] ZVEC_TYPENAME(Name)                        \
      zvec_from_array_##Name(const T *arr, size_t count);                      \
                                                                               \
  ZVEC_FUN_ATTRIBUTES [[nodiscard]] ZvecResult zvec_reserve_##Name(            \
      ZVEC_TYPENAME(Name) * v, size_t new_cap);                                \
                                                                               \
  ZVEC_FUN_ATTRIBUTES [[nodiscard]] ZvecResult                                 \
  zvec_allocate_uninitialized_##Name(ZVEC_TYPENAME(Name) * v,                  \
                                     size_t new_length);                       \
                                                                               \
  ZVEC_FUN_ATTRIBUTES                                                          \
      [[nodiscard]] T *zvec_push_slot_##Name(ZVEC_TYPENAME(Name) * v);         \
                                                                               \
  ZVEC_FUN_ATTRIBUTES [[nodiscard]] ZvecResult zvec_push_##Name(               \
      ZVEC_TYPENAME(Name) * v, T value);                                       \
                                                                               \
  ZVEC_FUN_ATTRIBUTES [[nodiscard]] ZvecResult zvec_extend_##Name(             \
      ZVEC_TYPENAME(Name) * v, const T *items, size_t count);                  \
                                                                               \
  ZVEC_FUN_ATTRIBUTES void zvec_pop_##Name(ZVEC_TYPENAME(Name) * v);           \
                                                                               \
  ZVEC_FUN_ATTRIBUTES                                                          \
      [[nodiscard]] T zvec_pop_get_##Name(ZVEC_TYPENAME(Name) * v);            \
                                                                               \
  ZVEC_FUN_ATTRIBUTES void zvec_shrink_to_fit_##Name(ZVEC_TYPENAME(Name) * v); \
                                                                               \
  ZVEC_FUN_ATTRIBUTES [[nodiscard]] T zvec_at_##Name(ZVEC_TYPENAME(Name) v,    \
                                                     size_t index);            \
                                                                               \
  ZVEC_FUN_ATTRIBUTES [[nodiscard]] T *zvec_get_at_mut_##Name(                 \
      ZVEC_TYPENAME(Name) * v, size_t index);                                  \
                                                                               \
  ZVEC_FUN_ATTRIBUTES [[nodiscard]] ZvecResult zvec_set_at_##Name(             \
      ZVEC_TYPENAME(Name) * v, size_t index, T value);                         \
                                                                               \
  ZVEC_FUN_ATTRIBUTES [[nodiscard]] T const *zvec_get_at_##Name(               \
      const ZVEC_TYPENAME(Name) * v, size_t index);                            \
                                                                               \
  ZVEC_FUN_ATTRIBUTES                                                          \
      [[nodiscard]] T *zvec_data_##Name(ZVEC_TYPENAME(Name) * v);              \
                                                                               \
  ZVEC_FUN_ATTRIBUTES                                                          \
      [[nodiscard]] T *zvec_last_##Name(ZVEC_TYPENAME(Name) * v);              \
                                                                               \
  ZVEC_FUN_ATTRIBUTES void zvec_remove_##Name(ZVEC_TYPENAME(Name) * v,         \
                                              size_t index);                   \
                                                                               \
  ZVEC_FUN_ATTRIBUTES void zvec_swap_remove_##Name(ZVEC_TYPENAME(Name) * v,    \
                                                   size_t index);              \
                                                                               \
  ...
  ZVEC_FUN_ATTRIBUTES void zvec_reverse_##Name(ZVEC_TYPENAME(Name) * v);       \
                                                                               \
  ZVEC_FUN_ATTRIBUTES void zvec_sort_##Name(                                   \
      ZVEC_TYPENAME(Name) * v, int (*compar)(T const *, T const *));           \
                                                                               \
  ZVEC_FUN_ATTRIBUTES [[nodiscard]] T *zvec_bsearch_##Name(                    \
      const ZVEC_TYPENAME(Name) * v, const T *key,                             \
      int (*compar)(T const *, T const *));                                    \
                                                                               \
  ZVEC_FUN_ATTRIBUTES [[nodiscard]] T *zvec_lower_bound_##Name(                \
      const ZVEC_TYPENAME(Name) * v, T const *key,                             \
      int (*compar)(T const *, T const *));

#if defined(__cplusplus)
#define ZVEC_EMPTY(TypeName) (ZVEC_TYPENAME(TypeName){NULL, 0, 0})
#else
#define ZVEC_EMPTY(TypeName)                                                   \
  ((ZVEC_TYPENAME(TypeName)){.data = NULL, .length = 0, .capacity = 0})
#endif

#define ZVEC_LENGTH(v) (v).length
#define ZVEC_IS_EMPTY(v) ((v).length == 0)

#define ZVEC_ASSERT_SHOULD_USE_PUSH(val)                                       \
  STATIC_ASSERT(sizeof(val) <= 8, "only small values should use push, use "    \
                                  "push slot for larger ones instead!")

#define ZVEC_ASSERT_SHOULD_USE_PUSH_SLOT(val)                                  \
...
#define ZVEC_ALLOCATE_UNINITIALIZED(T, v, length)                              \
  ZVEC_ALLOCATE_UNINITIALIZED_EXTENDED(T, T, v, length)
...
#define ZVEC_LOWER_BOUND(T, v, key, cmp)                                       \
  ZVEC_LOWER_BOUND_EXTENDED(T, T, v, key, cmp)
...
#define ZVEC_EXTEND_EXTENDED(T, Name, v, arr, count)                           \
  zvec_extend_##Name(v, arr, count)
#define ZVEC_RESERVE_EXTENDED(T, Name, v, cap) zvec_reserve_##Name(v, cap)
#define ZVEC_ALLOCATE_UNINITIALIZED_EXTENDED(T, Name, v, length)               \
  zvec_allocate_uninitialized_##Name(v, length)
...
#define ZVEC_SET_AT_EXTENDED(T, Name, v, idx, value)                           \
  zvec_set_at_##Name(v, idx, value)
...
#define ZVEC_BSEARCH_EXTENDED(T, Name, v, key, cmp)                            \
  zvec_bsearch_##Name(v, key, cmp)
#define ZVEC_LOWER_BOUND_EXTENDED(T, Name, v, key, cmp)                        \
  zvec_lower_bound_##Name(v, key, cmp)
...
#define ZVEC_PUSH_SLOT_ENTRY(T, Name)                                          \
  ZVEC_TYPENAME(Name) * : zvec_push_slot_##Name,
...
#define ZVEC_GET_AT_MUT_ENTRY(T, Name)                                         \
  ZVEC_TYPENAME(Name) * : zvec_get_at_mut_##Name,
...
#define ZVEC_SHRINK_ENTRY(T, Name)                                             \
  ZVEC_TYPENAME(Name) * : zvec_shrink_to_fit_##Name,
#define ZVEC_REMOVE_ENTRY(T, Name) ZVEC_TYPENAME(Name) * : zvec_remove_##Name,
#define ZVEC_SWAP_REM_ENTRY(T, Name)                                           \
  ZVEC_TYPENAME(Name) * : zvec_swap_remove_##Name,
...
#define ZVEC_LOWER_BOUND_ENTRY(T, Name)                                        \
  ZVEC_TYPENAME(Name) * : zvec_lower_bound_##Name,

#define zvec_push(v, val)                                                      \
  _Generic((v), REGISTER_TYPES(ZVEC_PUSH_ENTRY) default: 0)(v, val)
#define zvec_push_slot(v)                                                      \
  _Generic((v), REGISTER_TYPES(ZVEC_PUSH_SLOT_ENTRY) default: (void *)0)(v)
#define zvec_extend(v, arr, count)                                             \
  _Generic((v), REGISTER_TYPES(ZVEC_EXTEND_ENTRY) default: 0)(v, arr, count)
#define zvec_reserve(v, cap)                                                   \
  _Generic((v), REGISTER_TYPES(ZVEC_RESERVE_ENTRY) default: 0)(v, cap)
#define zvec_at(v, idx)                                                        \
  _Generic((v), REGISTER_TYPES(ZVEC_AT_ENTRY) default: (void *)0)(v, idx)
#define zvec_get_at(v, idx)                                                    \
  _Generic((v), REGISTER_TYPES(ZVEC_GET_AT_ENTRY) default: (void)0)(v, idx)
#define zvec_get_at_mut(v, idx)                                                \
  _Generic((v), REGISTER_TYPES(ZVEC_GET_AT_MUT_ENTRY) default: (void)0)(v, idx)
#define zvec_data(v)                                                           \
  _Generic((v), REGISTER_TYPES(ZVEC_DATA_ENTRY) default: (void *)0)(v)
#define zvec_last(v)                                                           \
  _Generic((v), REGISTER_TYPES(ZVEC_LAST_ENTRY) default: (void *)0)(v)
#define zvec_free(v)                                                           \
  _Generic((v), REGISTER_TYPES(ZVEC_FREE_ENTRY) default: (void)0)(v)
#define zvec_pop(v)                                                            \
  _Generic((v), REGISTER_TYPES(ZVEC_POP_ENTRY) default: (void)0)(v)
#define zvec_pop_get(v)                                                        \
  _Generic((v), REGISTER_TYPES(ZVEC_POP_GET_ENTRY) default: (void)0)(v)
#define zvec_shrink_to_fit(v)                                                  \
  _Generic((v), REGISTER_TYPES(ZVEC_SHRINK_ENTRY) default: (void)0)(v)
#define zvec_remove(v, i)                                                      \
  _Generic((v), REGISTER_TYPES(ZVEC_REMOVE_ENTRY) default: (void)0)(v, i)
#define zvec_swap_remove(v, i)                                                 \
  _Generic((v), REGISTER_TYPES(ZVEC_SWAP_REM_ENTRY) default: (void)0)(v, i)
#define zvec_clear(v)                                                          \
  _Generic((v), REGISTER_TYPES(ZVEC_CLEAR_ENTRY) default: (void)0)(v)
#define zvec_reverse(v)                                                        \
  _Generic((v), REGISTER_TYPES(ZVEC_REVERSE_ENTRY) default: (void)0)(v)
#define zvec_sort(v, cmp)                                                      \
  _Generic((v), REGISTER_TYPES(ZVEC_SORT_ENTRY) default: (void)0)(v, cmp)
#define zvec_bsearch(v, key, cmp)                                              \
  _Generic((v), REGISTER_TYPES(ZVEC_BSEARCH_ENTRY) default: (void *)0)(v, key, \
                                                                       cmp)
#define zvec_lower_bound(v, k, c)                                              \
  _Generic((v),                                                                \
      REGISTER_TYPES(ZVEC_LOWER_BOUND_ENTRY) default: (void *)0)(v, k, c)
...
#define zvec_from(Name, ...)                                                   \
  zvec_from_array_##Name((ZVEC_ELEMENT_TYPENAME(Name)[])__VA_ARGS__,           \
                         sizeof((ZVEC_ELEMENT_TYPENAME(Name)[])__VA_ARGS__) /  \
                             sizeof(ZVEC_ELEMENT_TYPENAME(Name)))

#define zvec_foreach(v, iter)                                                  \
  for (size_t ZVEC_NAME(_i_, __LINE__) = 0;                                    \
       ZVEC_NAME(_i_, __LINE__) < (v)->length &&                               \
       ((iter) = &(v)->data[ZVEC_NAME(_i_, __LINE__)]);                        \
...
#define ZVEC_IMPLEMENT_VEC_TYPE_EXTENDED(T, Name)                              \
  ZVEC_FUN_ATTRIBUTES [[nodiscard]] ZVEC_TYPENAME(Name)                        \
    ...
    return v;                                                                  \
                                                                               ...
                                                                               \
  ZVEC_FUN_ATTRIBUTES [[nodiscard]] ZVEC_TYPENAME(Name)                        \
      zvec_from_array_##Name(const T *const arr, size_t count) {               \
  ...
  zvec_allocate_uninitialized_##Name(ZVEC_TYPENAME(Name) * v,                  \
    ...
    return ZvecResultOk;                                                       \
                                                                               ...
                                                                               \
  ZVEC_FUN_ATTRIBUTES [[nodiscard]] T *zvec_push_slot_##Name(                  \
      ZVEC_TYPENAME(Name) * v) {                                               \
    if (v->length >= v->capacity) {                                            \
      size_t new_cap = v->capacity == 0 ? 8 : v->capacity * 2;                 \
      if (zvec_reserve_##Name(v, new_cap) != ZvecResultOk)                     \
        return NULL;                                                           \
    }                                                                          \
  ...
  ZVEC_FUN_ATTRIBUTES [[nodiscard]] ZvecResult zvec_push_##Name(               \
    ...
    *slot = value;                                                             \
    return ZvecResultOk;                                                       \
                                                                               ...
                                                                               \
  ZVEC_FUN_ATTRIBUTES [[nodiscard]] ZvecResult zvec_extend_##Name(             \
      ZVEC_TYPENAME(Name) * v, const T *const items, size_t count) {           \
  ...
  ZVEC_FUN_ATTRIBUTES void zvec_pop_##Name(ZVEC_TYPENAME(Name) * v) {          \
    ...
    v->length--;                                                               \
                                                                               ...
                                                                               \
  ZVEC_FUN_ATTRIBUTES [[nodiscard]] T zvec_pop_get_##Name(                     \
      ZVEC_TYPENAME(Name) * v) {                                               \
    assert(v->length > 0 && "Vector is empty, cannot pop!");                   \
    return v->data[--v->length];                                               \
  }                                                                            \
  ZVEC_FUN_ATTRIBUTES void zvec_shrink_to_fit_##Name(ZVEC_TYPENAME(Name) *     \
                                                     v) {                      \
    if (v->length == 0) {                                                      \
      Z_VEC_FREE(v->data);                                                     \
      *v = ZVEC_EMPTY(Name);                                                   \
  ...
  ZVEC_FUN_ATTRIBUTES [[nodiscard]] ZvecResult zvec_set_at_##Name(             \
    ...
    return ZvecResultOk;                                                       \
                                                                               ...
                                                                               \
  ZVEC_FUN_ATTRIBUTES [[nodiscard]] T const *zvec_get_at_##Name(               \
      ZVEC_TYPENAME(Name) const *const v, size_t index) {                      \
    return (index < v->length) ? &v->data[index] : NULL;                       \
  }                                                                            \
                                                                               \
  ZVEC_FUN_ATTRIBUTES [[nodiscard]] T *zvec_data_##Name(ZVEC_TYPENAME(Name) *  \
                                                        v) {                   \
    return v->data;                                                            \
  }                                                                            \
                                                                               \
  ZVEC_FUN_ATTRIBUTES [[nodiscard]] T *zvec_last_##Name(ZVEC_TYPENAME(Name) *  \
                                                        v) {                   \
  ...
  ZVEC_FUN_ATTRIBUTES void zvec_sort_##Name(                                   \
    ...
    if (v->length > 1) {                                                       \
      ...
      qsort(v->data, v->length, sizeof(T), qsort_cmp);                         \
                                                                               ...
                                                                               \
  ZVEC_FUN_ATTRIBUTES [[nodiscard]] T *zvec_bsearch_##Name(                    \
      const ZVEC_TYPENAME(Name) *const v, const T *key,                        \
      int (*compar)(T const *, T const *)) {                                   \
    if (v->length == 0)                                                        \
      return NULL;                                                             \
    ...
    return (T *)bsearch(key, v->data, v->length, sizeof(T), bs_cmp);           \
                                                                               ...
                                                                               \
  ZVEC_FUN_ATTRIBUTES [[nodiscard]] T *zvec_lower_bound_##Name(                \
      const ZVEC_TYPENAME(Name) *const v, T const *key,                        \
      int (*compar)(T const *, T const *)) {                                   \
    size_t l = 0;                                                              \
    size_t r = v->length;                                                      \
    while (l < r) {                                                            \
